...
#66 - 성공    
words = r"\b\w*?(\w{5,})\w*?\b.{1,30}?\b\w*?(\w{5,})\w*?\b.{1,130}?" + "     " + r".{1,130}?\b\w*?\1\w*?\b.{1,130}?\b\w*?\2\w*?\b"
belong = "章外_2章間_순차중첩_5_5_"
words_han ="_동일구조반복_그룹간거리(130)"

#2222 -     
words = r"\b\w*?(\w{3,})\w*?\b.{1,30}?\b\w*?(\w{3,})\w*?\b.{1,30}?\b\w*?(\w{3,})\w*?\b.{1,30}?\b\w*?(\w{3,})\w*?\b.{1,100}?" + \
    "     " + \
        r".{1,90}?\b\w*?\1\w*?\b.{1,30}?\b\w*?\2\w*?\b.{1,30}?\b\w*?\3\w*?\b.{1,30}?\b\w*?\4\w*?\b"
belong = "章外_2章間_순차중첩_2222_"
words_han ="_동일구조반복_그룹간거리(30)"
...
def sub_fff_TO_fff(count_number,w_num, 비교항 ,author1,author_year, book1, book1_year,반복항, line1, line2, line3, fff_list):
    count_number+=1
    sub_fff=[]
...
try:
    for e_f in data_files:
        ...
        with codecs.open(e_f,"r", encoding="utf-16") as f:
            ...
            f_lines = [line0102, line0203, line0103,line03_original]
                ...
                if len(line02_original)> 26:  #중장이 긴 것은 패스
                    ...
                    for each_line in f_lines:
                        ...
                        if len(c)==0 : #검색 값이 없을 때
                            ...
                            count_item, fff = sub_fff_TO_fff(count_item,work_num,비교항변수,author,author_year, book, book_year, 반복항변수, line01, line02, line03,fff) 
