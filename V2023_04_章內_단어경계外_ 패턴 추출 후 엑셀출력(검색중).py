...
#章內_단어경계外_중첩_5A_5B_5A_5B_
words = r"\b\w*?(\w{5,})\w*?\b.{1,30}?\b\w*?(\w{5,})\w*?\b.{1,30}?\b\w*?\1\w*?\b.{1,30}?\b\w*?\2\w*?\b"
#章內_단어경계外_중첩_5A_5A_5B_5B_
words = r"\b\w*?(\w{4,})\w*?\b.{1,30}?\b\w*?\1\w*?\b.{1,30}?\b\w*?(\w{4,})\w*?\b.{1,30}?\b\w*?\2\w*?\b"
#章內_단어경계外_중첩_5A_5A_5B_5B_패턴이 어절말
words = r"\b\w*?(\w{3,})\b.{1,30}?\b\w*?\1\w*?\b.{1,30}?\b\w*?(\w{3,})\b.{1,30}?\b\w*?\2\w*?\b"

words_han ="_동일어휘들의_반복_그룹간거리(30)"
belong = "송성57초_"
words_han ="_옛사랑_이제사랑_어제교태_오늘교태"

#章內_단어경계外_중첩___A___A_
words = r"\b\w*?(\w{3,5})\b.{1,3}?\b\w*?\1\b"
words_han ="_동일어휘들의_반복_그룹간거리(30)"
belong = "송성3중_"
words_han ="_뉘손대 나타관대"

#章內_단어경계外_중첩 A___A___
words = r"\b(\w{3,})\w*?\b\s\b\1\w*?\b"
words_han ="_동일_어두_반복_그룹간거리(30)"
belong = "송성55중_"
words_han ="_다료이 다려"



#章內_단어경계外_중첩 A_B_C_A_B_C_
words = r"\b\w*?(\w{2,})\w*?\b.{1,35}?\b\w*?(\w{2,})\w*?\b.{1,35}?\b\w*?(\w{2,})\w*?\b.{1,35}?\b\w*?\1\w*?\b.{1,35}?\b\w*?\2\w*?\b.{1,35}?\b\w*?\3\w*?\b"
words_han ="_A_B_C_A_B_C_"
belong = "송성1초_"
words_han ="_(2음소)아바님 날 나하시고 어마님 날 기라시니"
...
def sub_fff_TO_fff(count_number,w_num, 비교항 ,author1,author_year, book1, book1_year,반복항, line1, line2, line3, copynum,each_line11, fff_list):
    count_number+=1
    sub_fff=[]
    sub_fff.append(each_line11)
...
try:
    for e_f in data_files:
        ...
        with codecs.open(e_f,"r", encoding="utf-16") as f:
            ...
            f_originals = [line01_original,line02_original,line03_original]
            #f_lines = [line01_divided, line02_divided, line03_divided,line03_original]
            f_lines = [line01_divided, line02_divided, line03_divided]
            #print(name0)
            print(name)
            #time.sleep(1)
                ...
                if len(line02_original)> 26:  #중장이 긴 것은 패스
                    ...
                    for each_line in f_lines:
                        ...
                        if len(c)==0 : #검색 값이 없을 때
                            ...
                            반복항변수 =""
                            each_line_original = f_originals[each_line_count-1]
                            for i in c:
                                if 반복항변수 == "":
                                    반복항변수 = str(i)
                                else:
                            ...
                            count_item, fff = sub_fff_TO_fff(count_item,work_num,비교항변수,author,author_year, book, book_year, 반복항변수, line01, line02, line03,copy_num, each_line_original, fff) 
    ...
    if not len(fff)==0 :
        ...
        for s_fff in fff:
            #제목 행 세팅
            ws.cell(column = 1, row = 1, value = "검색된 패턴").border = Border(left = Side(style = "thin"), right = Side(style = "thin"), top = Side(style = "thin"), bottom = Side(style = "thin"))
            ws.cell(column = 1, row = 1, value = "검색된 패턴").font = Font(name="맑은 고딕", size=10, color="00FF0000")
        ...
        wb.save('C:\\Users\\park9\\OneDrive\\바탕 화면\\시조_수사_전승\\sizo_scan\\result\\' + belong +'result_' +
