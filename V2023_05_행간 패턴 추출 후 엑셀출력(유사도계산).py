...
# 유사도 30% 이상 기준으로 검사 결과 5000건 이상/ 쓸데없음.
...
words = r"\b\w*?(\w{3,})\w*?\b\W*?\b\w*?\1\w*?\b\W*?\w*?" + "     " + r"\w*?\b\w*?(\w{3,})\w*?\b\W*?\w*?\b\w*?\2\w*?\b"
words_han ="_유사도 검사30프로이상"
...
def sub_fff_TO_fff(count_number,w_num, 비교항 ,author1,author_year, book1, book1_year,ratios, line1, line2, line3, fff_list):
    count_number+=1
    sub_fff=[]
...
def meta_funtion(count_item1, work_num, 비교항변수1, ratio, fff2):
    ...
    lline01 = Sizo_Dic.sizo_dic['교본역대시조전서'][work_num]['초장']
    lline02 = Sizo_Dic.sizo_dic['교본역대시조전서'][work_num]['중장']
    lline03 = Sizo_Dic.sizo_dic['교본역대시조전서'][work_num]['종장']

    count_item1, fff2 = sub_fff_TO_fff(
        count_item1, work_num, 비교항변수1, author, author_year, book, book_year, ratio, lline01, lline02, lline03, fff2)
    return count_item1, fff2
...
try:
    for e_f in data_files:
        ...
        with codecs.open(e_f,"r", encoding="utf-16") as f:
            ...
            try:
                ...
                dist1 = difflib.SequenceMatcher(None, line01_divided, line02_divided).ratio()
                dist2 = difflib.SequenceMatcher(None, line01_divided, line03_divided).ratio()
                dist3 = difflib.SequenceMatcher(None, line02_divided, line03_divided).ratio()
                ...
                if dist1==0 or dist2==0 or dist3==0 : #검색 값이 없을 때
                    pass
                elif dist1>0.3 : #자모열에서 검색 값이 있을 때,
                    print('work_num :' + str(work_num))
                    count_item+=1
                    비교항변수 = "초01-중02"  
                    count_item, fff = meta_funtion(count_item, work_num, 비교항변수, dist1, fff)

                elif dist2 > 0.3:  # 자모열에서 검색 값이 있을 때,
                    print('work_num :' + str(work_num))
                    count_item += 1
                    비교항변수 = "중02-종03"
                    count_item, fff = meta_funtion(
                        count_item, work_num, 비교항변수,dist2, fff)

                elif dist3 > 0.3:  # 자모열에서 검색 값이 있을 때,
                    print('work_num :' + str(work_num))
                    count_item += 1
                    비교항변수 = "초01-종03"
                    #비교항변수 = "초01-중02-종03"
                    ...
                    count_item, fff = meta_funtion(
                        count_item, work_num, 비교항변수,dist3, fff)
    ...
    if not len(fff)==0 :
        ...
        for s_fff in fff:
            ...
            ws.cell(column = 3, row = 1, value = "비교항").border = Border(left = Side(style = "thin"), right = Side(style = "thin"), top = Side(style = "thin"), bottom = Side(style = "thin"))
            ws.cell(column = 3, row = 1, value = "비교항").font = Font(name="맑은 고딕", size=12, color="00FF0000")
            ...
            ws.cell(column = 8, row = 1, value = "유사도").border = Border(left = Side(style = "thin"), right = Side(style = "thin"), top = Side(style = "thin"), bottom = Side(style = "thin"))
            ws.cell(column = 8, row = 1, value = "유사도").font = Font(name="맑은 고딕", size=10, color="00FF0000")
        ...
        wb.save('C:\\Users\\park9\\OneDrive\\바탕 화면\\시조_수사_전승\\sizo_scan\\result_pattern\\result_' +
