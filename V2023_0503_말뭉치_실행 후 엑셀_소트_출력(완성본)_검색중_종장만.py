...
# ㄷㅏㄱ?ㅏ> .{3,25}ㄴ.ㄴㄷㅡㅅ" # 성공 "ㄴ[ㅏ|ᆞ]ㄴ\s\S.{1,15}ㅇㅏ\s"    /ㅇㅔㅅㅕ.{1,9}ㄹ\s?ㅅㅗㄴㅑ/
#words = r"\b\w*?(\w{4,6})\b.{1,3}?\b\w*?\1\b"  #"ㅅ\s.{2,20}ㄴ .{3,12}ㅇ[ㅖ|ㅔ]"
#  r"(?<!apple\s)\bpineapple\b"
       #r"(?!ㅎᆞㄴㅗㄹㅏ\r$)\b\w*[ㅏㅑ]\b.{1,3}\bㅎ\w*\b"
words = "^[ㄷ|ㅈ][ㅓ|ㅕ].{2,12}ㅇ[ㅏ|ㅑ] .{5,45}[ㅏ|ㅑ]\r$"
words = "ㅇㅓ.?ㄷㅣ\r$"
words = "ㅂㅜㄴㄴ[ㅔ|ㅐ]"
words = "ㅇㅓㄷ.{1,3}ㄷㅜㄱㅗ"
belong = "송성60_"
...
words = "ㄴ\s?[ㄷ|ㅈ][ㅕ|ㅓ].{2,12}ㅇ[ㅏ|ㅑ]\s?\r"
belong = "소환형 시조_"
words_han ="_ㄴ_뎌_00아(야)"
...
try:
                ...
                for each_line in f:
                    ...
                    if len(c)==0 and (grab%2) != 0  : #검색 값이 없을 때, 선행 행의 값을 가져오지 않음  
                        ...
                        if i==0: # 문서의 처음/Sizo_Dic과 연동하여 '작가명/창작연대' 변수로 가져올 것
                            ...
                            genre02 = Sizo_Dic.sizo_dic['교본역대시조전서'][work_num]['장르']
    ...
    if not len(fff)==0 :
        ...
        for s_fff in fff:
            ...
            ws.cell(column = 3, row = 1, value = "위치").border = Border(left = Side(style = "thin"), right = Side(style = "thin"), top = Side(style = "thin"), bottom = Side(style = "thin"))
            ws.cell(column = 3, row = 1, value = "위치").font = Font(name="맑은 고딕", size=12, color="00FF0000")
        ...
        combinated_filename = 'C:\\Users\\park9\\OneDrive\\바탕 화면\\시조_수사_전승\\sizo_scan\\result\\'+belong+'result_'+'('+str(count_item)+'건)'+words_han+"_from_"+mal+'.xlsx'
        wb.save(combinated_filename)
        wb.close
...
locale.setlocale(locale.LC_ALL, 'ko_KR.utf8')  # 한글 정렬을 위한 로케일 설정


# 엑셀 파일 불러오기
workbook = load_workbook(filename=combinated_filename)
...
sorted_data = []
for value_tuple in sorted(zip(col1_values, col2_values, data)):
    try:
        sorted_data.append(value_tuple[2])
    except TypeError:
        pass
...
filename = combinated_filename  # '파일이름1.xlsx'
sheetname = words_han  # 'Sheet1'
...
# 글자체와 크기 설정
font2 = Font(name='새굴림', size=12, bold=True)
...
#sorted_data = sorted(data, key=lambda x: (str(x[col1_idx]), x[col2_idx]))
sorted_data = sorted(data, key=lambda x: (str(x[col1_idx]), str(x[col2_idx])))


    
for i, row_data in enumerate(sorted_data):
    col1, col2, *other_columns_data = row_data
    row = [col1] + [col2] + other_columns_data[:] 
    for j, val in enumerate(row):
        cell = ws.cell(row=i+2, column=j+start_col, value=val)
        cell.border = border  # 테두리 스타일 추가
        if j == col1_idx or j == col2_idx:  # 칼럼1 또는 칼럼2
            cell.font = font1  # 글자체 스타일 추가
        elif j == col1_idx + 1 or j == col2_idx + 1:  # 칼럼1 또는 칼럼2 다음 칼럼
            cell.font = font2  # 글자체 스타일 추가
...
for col_num in range(1, max_col + 1):
    ...
    if col_num in [col1_idx+1, col2_idx+1]:  # 칼럼1과 칼럼2 다음 칼럼
        for cell in ws[col_letter]:
            cell.font = font2
    elif col_num in [col1_idx, col2_idx]:  # 칼럼1과 칼럼2
        for cell in ws[col_letter]:
            cell.font = font1
    else:
        for cell in ws[col_letter]:
...
for i, row in enumerate(ws.iter_rows(min_row=2)):
    if row[3].value == '鄭澈':  # 3번째 열(column)의 값이 '222'인 경우
        for cell in row:
            cell.fill = yellow_fill
            #cell.font = Font(color=YELLOW)
    else:
        pass
