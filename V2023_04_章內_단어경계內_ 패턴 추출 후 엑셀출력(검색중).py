...
#55 - 성공    
words = r"\b.{2,3}(\w{3,})\S+\1\w*?\b" 
words = r"\b.{2,3}(\w{3,})\1\b.{1,100}\b(\w{3,})\2.{1,10}?\b.{2,3}(\w{3,})\3\b.{1,50}\b(\w{3,})\4\b"

words = r"\b.{2,3}(\w{3,})\1\b.{1,100}\b.{2,3}(\w{3,})\2\b"
belong = "章內_단어경계內_중첩_3_3_"
words_han ="_동일어휘반복"
belong = "송성75초_"
words_han = "_화작작 범나븨 쌍쌍 류청청 괴꼬리 쌍쌍"

#같은 음절 4회 반복 -2음소 이상
words = r"\b(\w{2,3})\w*?\b.{1,30}?\b\1\w*?\b.{1,30}?\b\1\w*?\b.{1,30}?\b\1\w*?\b"
belong = "章內_단어경계외外_연쇄_2_2_2_2_"
words_han = "_동일어휘반복"
belong = "송성56중_"
words_han = "_곳 두고 달 두고 바회 두고 물 둔는이"
...
def sub_fff_TO_fff(count_number, w_num, 비교항, author1, author_year, book1, book1_year, 반복항, line1, line2, line3, copynum, each_line_original1, fff_list):
    count_number+=1
    sub_fff=[]
    sub_fff.append(each_line_original1)
...
try:
    ...
    if not len(fff)==0 :
        ...
        for s_fff in fff:
            #제목 행 세팅
            ws.cell(column = 1, row = 1, value = "결과").border = Border(left = Side(style = "thin"), right = Side(style = "thin"), top = Side(style = "thin"), bottom = Side(style = "thin"))
            ws.cell(column = 1, row = 1, value = "결과").font = Font(name="맑은 고딕", size=10, color="00FF0000")
